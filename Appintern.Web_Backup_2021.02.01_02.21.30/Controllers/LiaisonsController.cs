...
namespace Appintern.Web.Controllers
{
    ...
    /// </summary>
    public class LiaisonsController :    RenderMvcController
    {
        ...
        public new ActionResult Profile(ContentModel model, string memberAlias)
        {
            ...
            var members = memberService.GetMembersByPropertyValue(CM.Liaison.GetModelPropertyType(p => p.UrlSlug).Alias, memberAlias, StringPropertyMatchType.Exact);

            if (model.Content is CM.Liaisons membersNode && members.Count() == 1)
            {
