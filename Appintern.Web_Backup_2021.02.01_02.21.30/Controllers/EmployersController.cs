...
namespace Appintern.Web.Controllers
{
    ...
    /// </summary>
    public class EmployersController :    RenderMvcController
    {
        ...
        public new ActionResult Profile(ContentModel model, string memberAlias)
        {
            ...
            var members = memberService.GetMembersByPropertyValue(CM.Employer.GetModelPropertyType(p => p.UrlSlug).Alias, memberAlias, StringPropertyMatchType.Exact);

            if (model.Content is CM.Employers membersNode && members.Count() == 1)
            {
