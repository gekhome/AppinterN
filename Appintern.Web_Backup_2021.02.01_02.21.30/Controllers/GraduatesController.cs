...
namespace Appintern.Web.Controllers
{
    ...
    /// </summary>
    public class GraduatesController :    RenderMvcController
    {
        ...
        public new ActionResult Profile(ContentModel model, string memberAlias)
        {
            ...
            var members = memberService.GetMembersByPropertyValue(CM.Graduate.GetModelPropertyType(p => p.UrlSlug).Alias, memberAlias, StringPropertyMatchType.Exact);

            if (model.Content is CM.Graduates membersNode && members.Count() == 1)
            {
