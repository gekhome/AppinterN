...
namespace Appintern.Web.Controllers
{
    ...
    /// </summary>
    public class AmbassadorsController :    RenderMvcController
    {
        ...
        public new ActionResult Profile(ContentModel model, string memberAlias)
        {
            ...
            var members = memberService.GetMembersByPropertyValue(CM.Ambassador.GetModelPropertyType(p => p.UrlSlug).Alias, memberAlias, StringPropertyMatchType.Exact);

            if (model.Content is CM.Ambassadors membersNode && members.Count() == 1)
            {
