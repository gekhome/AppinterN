...
namespace Appintern.Web.Controllers
{
    ...
    /// </summary>
    public class SchoolsController :    RenderMvcController
    {
        ...
        public new ActionResult Profile(ContentModel model, string memberAlias)
        {
            ...
            var members = memberService.GetMembersByPropertyValue(CM.School.GetModelPropertyType(p => p.UrlSlug).Alias, memberAlias, StringPropertyMatchType.Exact);

            if (model.Content is CM.Schools membersNode && members.Count() == 1)
            {
