...
namespace Appintern.Web.Controllers
{
    ...
    /// </summary>
    public class StudentsController :    RenderMvcController
    {
        ...
        public new ActionResult Profile(ContentModel model, string memberAlias)
        {
            ...
            var members = memberService.GetMembersByPropertyValue(CM.Student.GetModelPropertyType(p => p.UrlSlug).Alias, memberAlias, StringPropertyMatchType.Exact);

            if (model.Content is CM.Students membersNode && members.Count() == 1)
            {
