...
namespace Appintern.Web.Controllers
{
    ...
    public class MembersController :    RenderMvcController
    {
        ...
        public new ActionResult Profile(ContentModel model, string memberAlias)
        {
            ...
            var members = memberService.GetMembersByPropertyValue(CM.Member.GetModelPropertyType(p => p.UrlSlug).Alias, memberAlias, StringPropertyMatchType.Exact);

            if (model.Content is CM.Members membersNode && members.Count() == 1)
            {
